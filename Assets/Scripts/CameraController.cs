...
public class CameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            Vector3 X = new Vector3(pointer.transform.position.x, 0, 0);
            Vector3 Y = new Vector3( 0, pointer.transform.position.y, 0);
            Vector3 Z = pointer.transform.position.z * squareToHex;

            Vector3 newHexPosition = X + Y + Z;
            newHexPosition = new Vector3((float)Math.Round(newHexPosition.x), newHexPosition.y, (float)Math.Round(newHexPosition.z));
            Debug.Log(newHexPosition);

            X = new Vector3(newHexPosition.x, 0, 0);
            Y = new Vector3(0, newHexPosition.y + 0.01f, 0);
            Z = newHexPosition.z * hexToSquare;

            Vector3 newSquarePosition = X + Y + Z;
            Instantiate(selectedHex, newSquarePosition, Quaternion.identity);
        }
